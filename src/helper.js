...
export function welcomeText(client) {
    packets.chat(client, [
        ...emojiFormat(
            '🟪➕=== Server Fallback di Newexpirence  ===⬜➖\n' +
            'Se sei qui vuol dire che il server principale è crashato \n' +
            'Per favore aspetta qui fino a quando si riaccendono i server \n' +
            'Questa libreria contiene una collezione di tutte le possibili combinazioni di 🟪➕15 caratteri⬜➖.\n\n' +
            'Fai tasto destro per aprire una libreria e prendere un libro.\n' +
            'Respawneranno quindi non serve rimetterli indietro.\n\n' +
            'Usa '
        ),
        { color: 'light_purple', text: '/help', bold: true, clickEvent: { action: 'suggest_command', value: `/help` } },
        ...emojiFormat(
            ' per ottenere una lista di tutti i comandi.\n\n'
        ),
        { color: 'light_purple', text: '/discord', bold: true, clickEvent: { action: 'suggest_command', value: `/discord` } },
        ...emojiFormat('\nEntra nel discord: '),
...
export function banKick(client, reason) {
    client.end(`§dSei stato bannato.${reason ? ` Ragione: ${reason}.` : ''}`);
}
...
export function error(client, message) {
    packets.chat(client, emojiFormat(`🟥➕[ERRORE]➖ ${message}`));
}
