...
server.on('login', function (client) {
    ...
    client.on('end', () => {
        ...
        broadcast(server.clients, 'chat', [emojiFormat(`${admins.has(client.uuid) ? '🟪' : '⬛'}${client.username}🟨 è uscito dal server.`)]);
        console.log(`${client.username} è uscito dal server (${Object.keys(server.clients).length}/${server.maxPlayers}).`);
    });

    client.__state.ready = true;

    broadcast(server.clients, 'chat', [emojiFormat(`${admins.has(client.uuid) ? '🟪' : '⬛'}${client.username}🟨 è entrato nel server.`)]);
    welcomeText(client);
});
